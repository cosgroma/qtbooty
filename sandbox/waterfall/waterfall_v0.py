...
# view = win.addViewBox()
# img = pg.ImageItem(autoLevels=False, autoDownsample=False)
# win.setWindowTitle('pyqtgraph example: Plotting')

img = pg.ImageView()
view = img.getView()
view.invertY(False)
view.setLimits(xMin=0, xMax=4096, yMin=0, yMax=100)
...
#   # if type(c) is PyQt4.QtGui.QGroupBox:
#   c.hide()
...
# rand = 2*np.random.random(length)
...
# lut = clrmp.getLookupTable(pmin - pdiff/2, 90, 1024)
# img.getImageItem().setLookupTable(lut, update=False)''
#
...
def update():
  ...
  # img.getImageItem().setRect(QtCore.QRect(0, 0, 1024, 1024))
